...
def parse_args():
    parser = argparse.ArgumentParser(description="Package FSClicker QML build for Windows distribution.")
    parser.add_argument("--build-dir", default="build-qml", help="CMake build directory.")
    ...
    parser.add_argument("--qml-dir", default="qml", help="Project QML source directory for windeployqt scanning.")
...
def deploy_qt(qt_bin, qml_dir, exe_path):
    if not qt_bin:
    ...
    cmd = [str(windeployqt_path), "--qmldir", str(qml_dir), str(exe_path)]
    print(f"运行命令: {' '.join(cmd)}")
    subprocess.run(cmd, check=True)
    print("Qt 与 QML 依赖库已部署完成")
...
def main():
    ...
    deploy_qt(qt_bin, qml_dir, packaged_exe_path)
